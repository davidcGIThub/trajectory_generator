...
def minimize_time_objective_function(variables, num_cont_pts, dimension):
    scale_factor = get_scale_factor(variables, num_cont_pts, dimension)
    objective = scale_factor**2
    return objective

def minimize_time_velocity_penalty_objective_function(variables, num_cont_pts, dimension):
    control_points = get_control_points(variables, num_cont_pts, dimension)
    velocity_cps =  control_points[:,0:-1] - control_points[:,1:]
    velocity_control_points_squared_sum = np.sum(velocity_cps**2,0)
    scale_factor = get_scale_factor(variables, num_cont_pts, dimension)
    objective = 100*scale_factor**2 - np.sum(velocity_control_points_squared_sum)
    return objective
