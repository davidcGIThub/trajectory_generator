...
for i in range(np.shape(start_pos_data)[1]-1):
    ...
    waypoint_2 = Waypoint(location=end_pos, velocity=end_vel)
    ...
    ax3.plot(time_data, velocity_data, label="velocity")
    ax3.plot(time_data, velocity_data*0 + max_velocity, label="max_velocity", color="k")
...
ax2.plot(start_pos_data[0,:], start_pos_data[1,:], color="k", alpha=0.4)
ax2.plot(end_pos_data[0,:], end_pos_data[1,:], color="tab:brown", alpha=0.4)
